...
struct dynarray* dynarray_create()
{
     ...
     * FIXED ME: 
     */
    //struct dynarray array = {.data = malloc(sizeof(int**) * 2), .size = 0, .capacity = 2};
...
void dynarray_insert(struct dynarray* da, void* val)
{
    ...
    if (da->size == da->capacity){
        void** temp = malloc(sizeof(void*) * (da->capacity * 2));
...
void dynarray_remove(struct dynarray* da, int idx)
{
     ...
     * FIXED ME: 
     */
    void** temp = malloc(sizeof(void*) * (da->size-1));
    int temp_index = 0;
    for(int i = 0; i < da->size; i++){
        if(i != idx){
            temp[temp_index] = da->data[i];
            temp_index++;
        }
    }
    free(da->data);
