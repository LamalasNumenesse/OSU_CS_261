...
int stack_isempty(struct stack* stack) {
	return (get_head(stack->list) == NULL) ? 1 : 0;
}
...
int compare(void* val, void* val1){
	int* val_int = val;
	int* val1_int = val1;
	return (val_int == val1_int) ? 0 : 1;
}
...
void* stack_pop(struct stack* stack) {
	void* head = stack_top(stack);
	list_remove(stack->list, head, &compare);
	return head;
}